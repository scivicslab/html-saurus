...
import com.scivicslab.htmlsaurus.MergedOcrClient.MarkerEquation;
...
class MergedOcrClientTest {
    ...
    void merge_replacesCoveredParagraphs_evenWhenOneEquationCoversTwo() {
        ...
        List<MarkerEquation> equations = List.of(
                new MarkerEquation(47.6, 162.1, 309.2, 189.6,
                        "$$x_1 = \\alpha' X + \\alpha'' Y$$ などとなり、これらの結果はすでに求めたものと一致している."),
                new MarkerEquation(126.7, 491.4, 273.1, 535.6, "$$F_X = \\alpha' F_1$$\n$$F_Y = \\alpha'' F_2$$")
        );
    ...
    void merge_equationCoveringNoParagraph_isInsertedByVerticalPosition() {
        ...
        List<MarkerEquation> equations = List.of(
                new MarkerEquation(18, 126, 180, 162, "$$E = mc^2$$"));
    ...
    void merge_equationBelowEveryParagraph_isAppended() {
        ...
        List<MarkerEquation> equations = List.of(
                new MarkerEquation(18, 200, 180, 250, "$$E = mc^2$$"));
    ...
    void merge_slightOverlap_doesNotReplaceTheParagraph() {
        ...
        List<MarkerEquation> equations = List.of(
                new MarkerEquation(0, 0, 180, 36, "$$x$$"));
    ...
    @Test
    void parseMarkerEquations_readsEquationBlocksFromTheDocumentTree() {
        String inner = """
        ...
        List<MarkerEquation> equations = MergedOcrClient.parseMarkerEquations(body);
    ...
    @Test
    void parseMarkerEquations_withoutOutput_returnsEmpty() {
        assertEquals(List.of(), MergedOcrClient.parseMarkerEquations("{\"success\": false}"));
    }
