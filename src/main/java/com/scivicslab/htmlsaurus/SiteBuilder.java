...
public class SiteBuilder {
    ...
    private SiteNode reorderFromDocusaurusConfig(SiteNode root) {
        ...
        try {
            ...
            // Extract sidebarId order and labels from docusaurus.config
            List<String> sidebarIdOrder = new ArrayList<>();
            Map<String, String> sidebarIdToLabel = new LinkedHashMap<>();
            var itemPattern = Pattern.compile(
                "sidebarId:\\s*['\"]([^'\"]+)['\"].*?label:\\s*['\"]([^'\"]+)['\"]",
                Pattern.DOTALL);
            var itemMatcher = itemPattern.matcher(configContent);
            while (itemMatcher.find()) {
                sidebarIdOrder.add(itemMatcher.group(1));
                sidebarIdToLabel.put(itemMatcher.group(1), itemMatcher.group(2));
            }

            // Extract sidebarId -> dirName from sidebars
            Map<String, String> sidebarIdToDirName = new HashMap<>();
            var sidebarPattern = Pattern.compile(
                "(\\w+):\\s*\\[\\{\\s*type:\\s*['\"]autogenerated['\"],\\s*dirName:\\s*['\"]([^'\"]+)['\"]");
            var sidebarMatcher = sidebarPattern.matcher(sidebarsContent);
            while (sidebarMatcher.find()) {
                sidebarIdToDirName.put(sidebarMatcher.group(1), sidebarMatcher.group(2));
            }

            // Build ordered dirName list with labels
            List<String> orderedDirNames = new ArrayList<>();
            Map<String, String> dirNameToLabel = new HashMap<>();
            for (String sidebarId : sidebarIdOrder) {
                String dirName = sidebarIdToDirName.get(sidebarId);
                if (dirName != null) {
                    orderedDirNames.add(dirName);
                    dirNameToLabel.put(dirName, sidebarIdToLabel.get(sidebarId));
                }
            }
