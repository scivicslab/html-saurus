...
## 追加: 複数階層の箇条書きで翻訳アコーディオンの位置がずれるバグ修正

ユーザー報告: ネストした箇条書きで翻訳の位置がおかしい。ダメなら箇条書き全体を
1つのアコーディオンにまとめる案も提示されたが、原因が単純だったので個別のまま修正した。

### 原因
`insertTranslation`の`LI`分岐が`el.appendChild(details)`で末尾に追加していたため、
ネストした`<ul>/<ol>`を持つ項目では、そのサブリスト全部の後ろに翻訳が付き、
対応する項目のテキストから離れた位置に表示されていた。

### 修正
`el.children`から直接の子である`UL`/`OL`を探し、`el.insertBefore(details, nestedList)`
でそのサブリストの直前（＝この項目自身のテキストの直後）に挿入するよう変更。
`nestedList`が無い項目では`insertBefore(details, null)`となり`appendChild`と同じ挙動。

### 検証
- 153テスト GREEN。
- 使い捨てインスタンスで2階層・3階層ネストのMarkdownをビルドし、生成されたHTMLの
  マークアップ構造（`<li>テキスト<ul>...</ul></li>`、`<p>`等での折り返しなし）を確認、
  各`<li>`が自分の直接の子リストだけを見るため多重ネストでも項目ごとに正しく効くことを確認。
  検証後に削除済み。

## 追加: Translateボタンをトグルにする

ユーザー要望: もう一度押すと翻訳アコーディオンが消えるようにしてほしい。

### 実装したもの
- `PageScripts.java`: クリックハンドラを`runTranslation()`（従来の翻訳フロー）と
  `clearTranslation()`（挿入した翻訳を全部消す）に分割し、`translated`フラグで分岐。
- `clearTranslation()`は `tr.translation-row`（表用に挿入した行）と
  `details.inline-translation`（段落・見出し・リスト項目用）を`remove()`、
  各ソース要素の`data-translated`を消して次回クリックで再スキャン対象に戻す。
  サーバ側キャッシュは消さないので、再度ONにしても2回目以降はほぼ即時。
- 表の挿入行だけを確実に区別するため、既存の`data-translated`頼みではなく
  `tr.className = 'translation-row'`を新たに付与（元データ行と誤って消さないため）。
- ボタン表示は既定「🌐 Translate」⇄ 翻訳済み「🌐 Hide translation」で状態を示す。

### 検証
- 153テスト GREEN、`mvn install`成功。
- 生成ページから実際のインラインJSを抽出し`node --check`で構文確認、
